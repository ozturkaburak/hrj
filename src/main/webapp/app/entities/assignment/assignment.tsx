...
export const Assignment = () => {
  ...
  const assignmentList = useAppSelector(state => state.hr.assignment.entities);
  const loading = useAppSelector(state => state.hr.assignment.loading);
  const totalItems = useAppSelector(state => state.hr.assignment.totalItems);
  ...
  return (
    <div>
      <h2 id="assignment-heading" data-cy="AssignmentHeading">
        <Translate contentKey="hrApp.assignment.home.title">Assignments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hrApp.assignment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/assignment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hrApp.assignment.home.createLabel">Create new Assignment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {assignmentList && assignmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="hrApp.assignment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('type')}>
                  <Translate contentKey="hrApp.assignment.type">Type</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('type')} />
                </th>
            ...
            <tbody>
              {assignmentList.map((assignment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{assignment.deletedAt ? <TextFormat type="date" value={assignment.deletedAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hrApp.assignment.home.notFound">No Assignments found</Translate>
            </div>
          )
        )}
      </div>
