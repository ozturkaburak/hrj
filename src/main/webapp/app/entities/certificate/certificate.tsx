...
export const Certificate = () => {
  ...
  const certificateList = useAppSelector(state => state.hr.certificate.entities);
  const loading = useAppSelector(state => state.hr.certificate.loading);
  const totalItems = useAppSelector(state => state.hr.certificate.totalItems);
  ...
  return (
    <div>
      <h2 id="certificate-heading" data-cy="CertificateHeading">
        <Translate contentKey="hrApp.certificate.home.title">Certificates</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hrApp.certificate.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/certificate/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hrApp.certificate.home.createLabel">Create new Certificate</Translate>
          </Link>
        </div>
      </h2>
        ...
        {certificateList && certificateList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="hrApp.certificate.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="hrApp.certificate.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                ...
                <th>
                  <Translate contentKey="hrApp.certificate.userProfile">User Profile</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {certificateList.map((certificate, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {certificate.updatedAt ? <TextFormat type="date" value={certificate.updatedAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {certificate.deletedAt ? <TextFormat type="date" value={certificate.deletedAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {certificate.userProfile ? (
                      <Link to={`/user-profile/${certificate.userProfile.id}`}>{certificate.userProfile.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hrApp.certificate.home.notFound">No Certificates found</Translate>
            </div>
          )
        )}
      </div>
