...
export const JobPosting = () => {
  ...
  const jobPostingList = useAppSelector(state => state.hr.jobPosting.entities);
  const loading = useAppSelector(state => state.hr.jobPosting.loading);
  const totalItems = useAppSelector(state => state.hr.jobPosting.totalItems);
  ...
  return (
    <div>
      <h2 id="job-posting-heading" data-cy="JobPostingHeading">
        <Translate contentKey="hrApp.jobPosting.home.title">Job Postings</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hrApp.jobPosting.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/job-posting/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hrApp.jobPosting.home.createLabel">Create new Job Posting</Translate>
          </Link>
        </div>
      </h2>
        ...
        {jobPostingList && jobPostingList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="hrApp.jobPosting.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
            ...
            <tbody>
              {jobPostingList.map((jobPosting, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/job-posting/${jobPosting.id}`} color="link" size="sm">
                      {jobPosting.id}
                    </Button>
                  </td>
                  ...
                  <td>
                    {jobPosting.createdDate ? <TextFormat type="date" value={jobPosting.createdDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/job-posting/${jobPosting.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/job-posting/${jobPosting.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hrApp.jobPosting.home.notFound">No Job Postings found</Translate>
            </div>
          )
        )}
      </div>
