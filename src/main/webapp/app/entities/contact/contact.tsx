...
export const Contact = () => {
  ...
  const contactList = useAppSelector(state => state.hr.contact.entities);
  const loading = useAppSelector(state => state.hr.contact.loading);
  const totalItems = useAppSelector(state => state.hr.contact.totalItems);
  ...
  return (
    <div>
      <h2 id="contact-heading" data-cy="ContactHeading">
        <Translate contentKey="hrApp.contact.home.title">Contacts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hrApp.contact.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/contact/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hrApp.contact.home.createLabel">Create new Contact</Translate>
          </Link>
        </div>
      </h2>
        ...
        {contactList && contactList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="hrApp.contact.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="hrApp.contact.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="hrApp.contact.city">City</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {contactList.map((contact, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{contact.deletedAt ? <TextFormat type="date" value={contact.deletedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{contact.user ? contact.user.id : ''}</td>
                  <td>{contact.city ? <Link to={`/city/${contact.city.id}`}>{contact.city.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="hrApp.contact.home.notFound">No Contacts found</Translate>
            </div>
          )
        )}
      </div>
